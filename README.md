...
A Python CLI and library for transforming XML documents (TEI, DocBook, …) using TEI Publisher Processing Model rules compiled from ODD files. Outputs HTML, Markdown, and DOCX.
...
uv run teipublisher transform modules/teipublisher-web.py demo/tei-test.xml \
  --preview --template templates/tufte.html.j2

# Generate a DOCX file
uv run teipublisher compile odd/teipublisher.odd --mode docx
uv run teipublisher transform modules/teipublisher-docx.py demo/tei-test.xml \
  -o output.docx
...
```bash
uv run teipublisher transform modules/teipublisher-web.py input.xml -o output.html
uv run teipublisher transform modules/teipublisher-docx.py input.xml -o output.docx
```

The transform module must define:

- `transform(root, options=None)` — run the processing model.
- `transform_output_channels()` — return the channel list (e.g. `['web']`, `['docx']`). Modules emitted by `teipublisher compile` include this automatically.

Arguments and options:

- `-m, --module PATH`: path to the transform `.py` file. Falls back to `transform.module` in `teipublisher.toml`.
...
- `--template PATH`: dual-purpose template path:
  ...
  - For **DOCX** output: path to a `.docx` file used as the Word style template. If omitted, falls back to `docx.template` in `teipublisher.toml`, then the built-in default.
...
- `--webcomponents / --no-webcomponents`: enable/disable tei-publisher web components mode. `alternate` behaviours emit `<pb-alternate>` and the document template loads `tei-publisher-components`. Falls back to `webcomponents.enabled` in `teipublisher.toml`.
- `-c, --config PATH`: path to a TOML configuration file (default: `teipublisher.toml` in the current directory).
...
uv run teipublisher transform modules/teipublisher-docx.py input.xml \
...
```bash
uv run teipublisher chunk input.xml -o chunks/
```

Configuration is primarily read from the `[chunking]` section of `teipublisher.toml` (see below). CLI options override config values.
...
- `-c, --config PATH`: TOML configuration file (default: `teipublisher.toml`).

Examples:

```bash
# Chunk using settings from teipublisher.toml
uv run teipublisher chunk input.xml

# Override output directory, force overwrite
uv run teipublisher chunk input.xml -o public/chunks --force

# Produce JSON data files for a static site generator
uv run teipublisher chunk input.xml --format json -o _data/chunks
```

## DOCX Output

When `transform_output_channels()` returns `['docx']`, `teipublisher transform` produces a `.docx` file (binary). The DOCX generator:
...
## Project Configuration (`teipublisher.toml`)

Place a `teipublisher.toml` file in the project root to set defaults that apply to all CLI commands. All sections are optional.
