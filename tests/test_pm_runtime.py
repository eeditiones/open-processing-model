...
def test_apply_template_param_value_expands_context_element_to_children() -> None:
    ...
    config = RenderContext(dispatch=dispatch)
...
def test_inject_cached_footnotes_appends_to_body_end() -> None:
    ...
    dl.set('id', 'fn_x')
    config = RenderContext(state=RunState(footnotes=[dl]))
...
def test_inject_cached_footnotes_preserves_order() -> None:
    ...
    dl2.set('id', 'fn_2')
    config = RenderContext(state=RunState(footnotes=[dl1, dl2]))
...
def test_inject_cached_footnotes_noop_when_empty() -> None:
    html = etree.Element('html')
    body = etree.SubElement(html, 'body')
    inject_cached_footnotes([html], RenderContext())
    assert len(body) == 0
...
def test_select_unwraps_singleton_count() -> None:
    """``count(...)`` is one atomic; a param must see an ``int``, not ``[int]``."""
    xml = f'''<TEI xmlns="{TEI_NS}"><text><body><div><div><head>t</head></div></div></body></text></TEI>'''
    root = etree.fromstring(xml.encode())
    head = root.find(f'.//{{{TEI_NS}}}head')
    assert head is not None
    n = XPathEnvironment().select(head, 'count(ancestor::div)')
    assert n == 2
    assert isinstance(n, int)


def test_select_returns_lxml_elements_not_wrappers() -> None:
    ...
    expan = env.select(choice, 'expan[1]')
    abbr = env.select(choice, 'abbr[1]')
...
def test_resolve_element_selects_single_node() -> None:
    xml = f'''<TEI xmlns="{TEI_NS}"><text><body><p>x</p><p>y</p></body></text></TEI>'''
    root = etree.fromstring(xml.encode())
    body = root.find(f'.//{{{TEI_NS}}}body')
    assert body is not None
    assert XPathEnvironment().resolve_element(root, '//body') is body


def test_resolve_element_requires_unique_element() -> None:
    xml = f'''<TEI xmlns="{TEI_NS}"><text><body><p>a</p><p>b</p></body></text></TEI>'''
    root = etree.fromstring(xml.encode())
    with pytest.raises(ValueError, match='exactly one'):
        XPathEnvironment().resolve_element(root, '//p')


def test_parent_axis_matches_namespaced_tei_elements() -> None:
    ...
    assert head is not None
    env = XPathEnvironment()
    assert env.test(head, 'parent::div') is True
    assert env.test(head, 'parent::figure') is False
...
def test_parameters_root_is_the_viewed_node_not_the_copy() -> None:
  ...
  <section xml:id="install"><title>Install</title>
    ...
    env = XPathEnvironment(root=install)

    assert env.select(
        intro, 'string(($parameters?root)/ancestor::article/info/title)',
    ) == 'Guide'
    assert env.select(intro, '$parameters?root is .') is False
...
def test_serialize_after_inject() -> None:
    ...
    dl = etree.Element('dl')
    dl.set('class', 'footnote')
    inject_cached_footnotes([html], RenderContext(state=RunState(footnotes=[dl])))
    s = serialize([html])
...
def test_document_uri_reports_the_source_file(tmp_path) -> None:
    ...
    env = XPathEnvironment(base_uri=uri, root=root)

    assert env.select(root, 'string(document-uri(root(.)))') == uri
    assert env.select(root, 'string(base-uri(root(.)))') == uri
    assert env.select(root, 'string(document-uri(root($parameters?root)))') == uri
...
def test_root_of_parameters_root_reaches_the_source_document(tmp_path) -> None:
    ...
    env = XPathEnvironment(base_uri=uri, root=source_div)
    pb = chunk.find(f'{{{TEI_NS}}}pb')

    assert env.select(pb, 'string(root($parameters?root)//teiHeader//title)') == 'Much Adoe'
    assert env.select(pb, 'count(root($parameters?root)//pb)') == 2
    assert env.select(pb, 'string(document-uri(root($parameters?root)))') == uri
...
def test_source_node_resolves_a_chunk_copy_to_the_stored_document() -> None:
    ...
    try:
        ...
        env = XPathEnvironment(root=doc)

        assert env.select(pb, 'count(tp:source-node(.)/preceding::pb) + 1') == 3
        assert env.select(pb, 'string(tp:source-node(.)/@n)') == '103'
        # Unmapped nodes pass through unchanged, so the identity case still works.
        assert env.select(doc, 'count(tp:source-node(.)//pb)') == 3
    finally:
        source_map.clear()
...
def test_source_node_is_identity_without_a_recorded_copy() -> None:
    ...
    assert XPathEnvironment().select(second, 'count(tp:source-node(.)/preceding::pb)') == 1
...
def test_apply_children_closes_up_a_word_split_at_a_soft_hyphen() -> None:
    ...
    el = etree.Element('p')
    config = RenderContext(dispatch=lambda *a, **k: [])
